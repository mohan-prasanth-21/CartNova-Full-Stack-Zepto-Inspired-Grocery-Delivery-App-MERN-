    ...
    <div className="bar-chart">
      {data.map((d, i) => (
        <div key={i} className="bar-col">
          ...
          <span className="bar-val">₹{d.value >= 1000 ? (d.value/1000).toFixed(1)+'k' : d.value}</span>
        </div>
      ))}
    </div>
  )
        ...
        {segments.map((seg, i) => {
          ...
          return (
            <circle key={i} cx="50" cy="50" r={R} fill="none" stroke={seg.color}
              ...
              strokeLinecap="butt" transform={`rotate(${rotate} 50 50)`} />
          )
        })}
        <text x="50" y="47" textAnchor="middle" fontSize="10" fontWeight="700" fill="#111">
          {segments[0]?.pct}%
        </text>
        <text x="50" y="57" textAnchor="middle" fontSize="7" fill="#666">Top cat</text>
      </svg>
...
const revenueData  = [
  { label: 'Mon', value: 18400 }, { label: 'Tue', value: 23200 }, { label: 'Wed', value: 19800 },
  { label: 'Thu', value: 29600 }, { label: 'Fri', value: 34200 }, { label: 'Sat', value: 41800 },
  { label: 'Sun', value: 38500 },
]
const monthlyData  = [
  { label: 'Jan', value: 280000 }, { label: 'Feb', value: 310000 }, { label: 'Mar', value: 295000 },
  { label: 'Apr', value: 380000 }, { label: 'May', value: 420000 }, { label: 'Jun', value: 395000 },
]
const catSegments = [
...
export default function Analytics() {
  ...
  const KPIS = [
    { label: 'Total Revenue',   value: `₹${totalRevenue >= 1000 ? (totalRevenue/1000).toFixed(1)+'k' : totalRevenue}`, delta: '+18%', icon: '💰', color: '#1a9e5c', bg: '#e8f8f0' },
    { label: 'Total Orders',    value: orders.length.toLocaleString(),   delta: '+12%', icon: '📦', color: '#3b82f6', bg: '#eff6ff' },
    { label: 'Active Users',    value: '8,430',  delta: '+24%', icon: '👥', color: '#8b5cf6', bg: '#f5f3ff' },
    { label: 'Avg Order Value', value: orders.length ? `₹${Math.round(totalRevenue/orders.length)}` : '₹0', delta: '+5%', icon: '🛒', color: '#f59e0b', bg: '#fffbeb' },
      ...
      <main className="admin-main">
          ...
          <div className="at-actions">
            <div className="range-tabs">
              {['week','month','year'].map(r => (
                <button key={r} className={`rtab ${range === r ? 'active' : ''}`} onClick={() => setRange(r)}>
        ...
        <div className="kpi-grid">
          {KPIS.map(k => (
            <div className="kpi-card" key={k.label}>
              ...
              <span className={`kpi-delta ${k.delta.startsWith('+') ? 'up' : 'dn'}`}>{k.delta}</span>
            </div>
          ))}
        </div>
        ...
        <div className="charts-row">
          <div className="chart-card big">
            <div className="chart-header">
              ...
              <span className="chart-badge">{range === 'week' ? 'This Week' : range === 'month' ? 'This Month' : 'This Year'}</span>
            </div>
            {range === 'week' ? <BarChart data={revenueData} color="#1a9e5c" /> : <LineChart data={monthlyData} color="#1a9e5c" />}
          </div>
            ...
            <div className="top-products">
              {topProducts.map((p, i) => (
                <div className="tp-row" key={p._id || p.id}>
                  <span className="tp-rank" style={{ color: ['#f59e0b','#9ca3af','#cd7f32','#555','#555','#555'][i] }}>#{i+1}</span>
                  <img src={p.image} alt={p.name} className="tp-img" onError={e => e.target.src = `https://placehold.co/36x36/e8f8f0/1a9e5c?text=${p.name[0]}`} />
          ...
          <div className="chart-card big">
            ...
            {recentOrders.length === 0 ? (
              <div className="no-orders-analytics">
                ...
                <p style={{fontSize:12,color:'var(--text-muted)',marginTop:4}}>Orders will appear here after checkout.</p>
              </div>
            ) : (
              <table className="orders-table">
                <thead>
                  <tr><th>Order ID</th><th>Items</th><th>City</th><th>Total</th><th>Status</th></tr>
                </thead>
                <tbody>
                  {recentOrders.map(o => (
                    <tr key={o._id}>
                      <td className="ot-id">#{o._id?.slice(-6).toUpperCase()}</td>
                      <td>{o.items?.length} items</td>
                      <td className="ot-addr">{o.address?.city || '—'}</td>
                      <td><b>₹{o.total}</b></td>
                      <td><span className={`ot-status ${o.status?.toLowerCase()}`}>{o.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="chart-card full">
          ...
          <table className="inv-table">
            <thead>
              <tr><th>#</th><th>Product</th><th>Category</th><th>Price</th><th>MRP</th><th>Discount</th><th>Rating</th><th>Status</th></tr>
            </thead>
            <tbody>
              {products.map((p, i) => (
                <tr key={p._id || p.id}>
