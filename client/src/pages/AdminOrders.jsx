    ...
    <div className="analytics-pg">
      ...
      <main className="admin-main">
        <div className="admin-topbar">
          <div>
            <h1>📦 Orders</h1>
            <p className="at-sub">{orders.length} total orders • ₹{totalRevenue} revenue</p>
          </div>
        </div>
        ...
        {selected && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: 'white', borderRadius: 16, padding: 28, width: '90%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h3 style={{ fontSize: 17, fontWeight: 800 }}>Order #{selected._id?.slice(-8).toUpperCase()}</h3>
              ...
              <div style={{ background: '#f8fafc', borderRadius: 10, padding: 14, marginBottom: 14 }}>
                ...
                <p style={{ fontSize: 13, color: '#374151' }}>{selected.address?.flat}, {selected.address?.area}, {selected.address?.city} — {selected.address?.pincode}</p>
              </div>
        ...
        <div className="chart-card full">
          ...
          {loading ? (
            ...
            <table className="orders-table">
              <thead>
                <tr><th>Order ID</th><th>Date</th><th>Items</th><th>City</th><th>Total</th><th>Payment</th><th>Status</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {filtered.map(o => (
                  <tr key={o._id}>
                    <td className="ot-id">#{o._id?.slice(-8).toUpperCase()}</td>
                    <td style={{ fontSize: 12, color: '#64748b' }}>{new Date(o.createdAt).toLocaleDateString('en-IN')}</td>
